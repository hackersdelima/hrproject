				...
				<table class="table table-bordered">
					<tbody>
						<tr>
							...
							<spring:url value="/verification/applicantactivate/${userid }" var="activate"/>
							<spring:url value="/verification/applicantdeactivate/${userid }" var="deactivate"/>
							<td>Action: <span><a class="btn btn-link" id="activate" href="${activate }">Activate</a></span> <span><a href="${deactivate }" class="btn btn-link" id="deactivate">Deactivate</a></span></td>
						</tr>
  ...
  <div class="modal-dialog">
	...
	angular.module("sa", []).controller('foo', function($scope, $http) {
		  ...
		  $scope.cashsummary = function() {
		    ...
		    var url = 'http://localhost:8080/hrproject/admin/applicants/'+userid;
	 ...
	 $.ajax({
		 type: "GET",
		 url: "http://localhost:8080/hrproject/document/findByUserid/"+userid,
		 success: function(result)
		 {
			 $("#image1").attr("src","data:image/png;base64,"+result.data[0].image+"");
			 $("#image2").attr("src","data:image/png;base64,"+result.data[1].image+"");
		 }
	 });
	</script>
	<script>
